...
function GetToolPath(const ToolName: string): string;
begin
  {$IFDEF WINDOWS}
  Result := ExtractFilePath(ParamStr(0)) + 'tools\' + ToolName + '.exe';
  {$ELSE}
  Result := '/usr/bin/' + ToolName;  // для Linux можно указать системный путь
  {$ENDIF}
end;

procedure HTMLToPDF(const HtmlFile, PdfFile: string);
var
  AProcess: TProcess;
  ToolPath: string;
begin
  ToolPath := GetToolPath('wkhtmltopdf');

  if not FileExists(ToolPath) then
  begin
    ShowMessage('Ошибка: не найден ' + ToolPath);
    Exit;
  end;

  AProcess := TProcess.Create(nil);
  try
    AProcess.Executable := ToolPath;
    AProcess.Parameters.Add(HtmlFile);
